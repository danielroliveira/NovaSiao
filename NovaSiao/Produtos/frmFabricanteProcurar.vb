...
Public Class frmFabricanteProcurar
    Private dtFab As DataTable = Nothing
...
#Region "LISTAGEM"
    ...
    Private Sub FormataListagem()
        ...
        '
        clnID.DisplayMember = "IDFabricante"
        clnFabricante.DisplayMember = "Fabricante"
        clnAtivo.DisplayMember = "FabricanteAtivo"
        clnAtivo.Width = 0
        '
...
#Region "BUTTONS FUNCTION"
    ...
    Private Sub btnEscolher_Click(sender As Object, e As EventArgs) Handles btnEscolher.Click
        ...
        If lstItens.SelectedItems.Count = 0 Then
            MessageBox.Show("Não nenhum FABRICANTE selecionado..." & vbNewLine &
                            "Favor antes selecione um FABRICANTE!",
                            "Escolher FABRICANTE", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If
        '
        ' DEFINIR O VALOR
        propIDFab_Escolha = CInt(lstItens.SelectedItems(0).Text) ' ID DO FABRICANTE
        propFab_Escolha = lstItens.SelectedItems(0).SubItems(1).Text ' DESCRICAO DO FABRICANTE
        '
...
#Region "OUTRAS FUNCOES"
    '
    '--- FAZ A SELECAO E A PROCURA QUANDO DIGITA O TEXTO
    ...
    Private Sub ProcurarST()
        '
        Dim dvFab As DataView = dtFab.DefaultView
        '
        If txtProcura.TextLength > 0 AndAlso IsNumeric(txtProcura.Text.Substring(0, 1)) Then
            dvFab.RowFilter = "IDFabricante = " & txtProcura.Text
        Else
            dvFab.RowFilter = "Fabricante LIKE '*" & txtProcura.Text & "*'" ' PROCURA PELO NOME
        End If
        '
    End Sub
    '
