...
#Region "BUTTONS FUNCTION"
	...
	Private Sub FinalizarCaixa()
		...
		Try
			...
			'
			'--- Finaliza o caixa
			cxBLL.CaixaFinalizar(_Caixa, lstMov, dbTran)
			'
...
#Region "FUNCOES IMPORTANTES"
	...
	Private Sub CalculaTotais()
		...
		Dim TS As Double = 0
		'
		E = lstMov.Where(Function(x) x.Mov = "E").Sum(Function(x) x.MovValorReal)
		S = lstMov.Where(Function(x) x.Mov = "S").Sum(Function(x) x.MovValorReal)
		TE = lstMov.Where(Function(x) x.Mov = "TE").Sum(Function(x) x.MovValorReal)
		TS = lstMov.Where(Function(x) x.Mov = "TS").Sum(Function(x) x.MovValorReal)
		...
		'
		_TTransf = TE - TS
		lblTTransf.Text = Format(Math.Abs(_TTransf), "C")
		'
		'--- Calcula SALDOS
		'--------------------------------------------------------------------------------------------
